<script lang="ts">
  ...
  export default class VueYandexShare extends Vue {
    ...
    /**
     * Признак того, что загрузка стилей отключена. Если
     * добавить атрибут, соцсети будут отображаться в виде текстового
    ...
    /**
     * Хэштеги. Актуальны и работают только для
     * Твиттера
     */
    @Prop({
      type: String,
    ...
    /**
     * Язык блока. Локализуются подписи кнопок соцсетей и
    ...
    /**
     * Расположение pop-up относительно контейнера
     * блока. Значение outer может понадобиться в том случае, если из-за
     * специфики верстки вашего сайта pop-up обрезается соседними элементами
     * страницы
     */
    @Prop({
      type: String,
